...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query(
        value = "select orderItem from OrderItem orderItem left join fetch orderItem.pillow left join fetch orderItem.defaultSize",
        countQuery = "select count(orderItem) from OrderItem orderItem"
    )
    Page<OrderItem> findAllWithToOneRelationships(Pageable pageable);

    @Query("select orderItem from OrderItem orderItem left join fetch orderItem.pillow left join fetch orderItem.defaultSize")
    List<OrderItem> findAllWithToOneRelationships();

    @Query(
        "select orderItem from OrderItem orderItem left join fetch orderItem.pillow left join fetch orderItem.defaultSize where orderItem.id =:id"
    )
